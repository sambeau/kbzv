...
describe('ProgressBar', () => {
  ...
  it('renders the label', () => {
    render(<ProgressBar done={5} total={10} percentage={50} label="Features" />);
  ...
  it('rounds the percentage', () => {
    render(<ProgressBar done={1} total={3} percentage={33.333} label="Tasks" />);
...
describe('EstimateDisplay', () => {
  it('renders entityEstimate when provided', () => {
    const rollup: EstimateRollup = { totalPoints: 5, estimatedCount: 1, unestimatedCount: 0 };
  ...
  it('renders "unestimated" when no items are estimated', () => {
    const rollup: EstimateRollup = { totalPoints: 0, estimatedCount: 0, unestimatedCount: 3 };
  ...
  it('renders rollup line when estimatedCount > 0', () => {
    const rollup: EstimateRollup = { totalPoints: 13, estimatedCount: 3, unestimatedCount: 1 };
    render(<EstimateDisplay rollup={rollup} />);
  ...
  it('renders nothing significant when all counts are 0', () => {
    const rollup: EstimateRollup = { totalPoints: 0, estimatedCount: 0, unestimatedCount: 0 };
...
describe('StatusBadge', () => {
  ...
  it('calls activateStatusFilter on click', () => {
    const activateFn = vi.fn();
    useUIStore.setState({ activateStatusFilter: activateFn });

    render(<StatusBadge status="done" />);
    fireEvent.click(screen.getByText('done'));
    expect(activateFn).toHaveBeenCalledWith('done');
  });

  it('renders "done" with green colour classes', () => {
    render(<StatusBadge status="done" />);
    const badge = screen.getByText('done').closest('div') ?? screen.getByText('done');
    expect(badge.className).toMatch(/green/);
  });

  it('renders "active" with yellow colour classes', () => {
    render(<StatusBadge status="active" />);
    const badge = screen.getByText('active').closest('div') ?? screen.getByText('active');
    expect(badge.className).toMatch(/yellow/);
  });

  it('renders "blocked" with orange colour classes', () => {
    render(<StatusBadge status="blocked" />);
    const badge = screen.getByText('blocked').closest('div') ?? screen.getByText('blocked');
