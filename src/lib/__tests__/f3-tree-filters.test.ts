...
function makePlanNode(plan: Plan, children: TreeNode[] = []): TreeNode {
...
describe("isVisible", () => {
  ...
  it("returns true when type and colour are both active", () => {
    expect(
      isVisible({ status: "active" }, "task", ALL_TYPES, ALL_COLOURS),
    ).toBe(true);
  });

  it("returns false when type is not active", () => {
    const noTasks = new Set(["plan", "feature"]);
    expect(isVisible({ status: "active" }, "task", noTasks, ALL_COLOURS)).toBe(
      false,
    );
  });

  it("returns false when status colour is not active", () => {
    const noYellow = new Set([
      "grey",
      "blue",
      "orange",
      "green",
    ...
    expect(isVisible({ status: "active" }, "task", ALL_TYPES, noYellow)).toBe(
      false,
    );
  });

  it("maps unknown status to grey and checks grey colour", () => {
    const noGrey = new Set([
      "blue",
      "yellow",
      "orange",
      "green",
    ...
    expect(
      isVisible({ status: "unknown-xyz" }, "task", ALL_TYPES, noGrey),
    ).toBe(false);
    expect(
      isVisible({ status: "unknown-xyz" }, "task", ALL_TYPES, ALL_COLOURS),
    ).toBe(true);
  });
});
...
describe("applyTreeFilters", () => {
  ...
  it("returns all nodes when no filters are active", () => {
    ...
    const tree: TreeNode[] = [
      makePlanNode(plan, [makeFeatureNode(feature, [makeTaskNode(task)])]),
    ];
  ...
  it("shows all nodes when type set is empty (empty = show all)", () => {
    const plan = makePlan({ id: "P1-test", status: "active" });
    const feature = makeFeature({
      id: "FEAT-001",
      parent: "P1-test",
  ...
  it("shows all nodes when both type and colour sets are empty", () => {
    const plan = makePlan({ id: "P1-test", status: "active" });
    const tree: TreeNode[] = [makePlanNode(plan, [])];

    const noTypes = new Set<string>();
    const noColours = new Set<string>();
    const result = applyTreeFilters(tree, noTypes, noColours);
    expect(result).toHaveLength(1);
    expect(result[0]._ghost).toBe(false);
  });

  it("filters to only checked types when set is non-empty", () => {
    const plan = makePlan({ id: "P1-test", status: "active" });
    const feature = makeFeature({
      id: "FEAT-001",
      parent: "P1-test",
    ...
    // Only "task" checked — plan and feature should be ghosts
    const onlyTask = new Set(["task"]);
    const result = applyTreeFilters(tree, onlyTask, new Set<string>());
    expect(result).toHaveLength(1);
    expect(result[0]._ghost).toBe(true); // plan is ghost
    expect(result[0].children[0]._ghost).toBe(true); // feature is ghost
    expect(result[0].children[0].children).toHaveLength(1); // task visible
  });
});
...
describe("ui-store — filter state", () => {
  const DEFAULT_TYPES = new Set(["plan", "feature", "task"]);
  const DEFAULT_COLOURS = new Set([
    "grey",
    "blue",
    "yellow",
    "orange",
  ...
  it("toggleType removes an active type", () => {
    useUIStore.getState().toggleType("plan");
    expect(useUIStore.getState().activeTypes.has("plan")).toBe(false);
  });

  it("toggleType re-adds an inactive type", () => {
    useUIStore.getState().toggleType("plan");
    useUIStore.getState().toggleType("plan");
    expect(useUIStore.getState().activeTypes.has("plan")).toBe(true);
  });
  ...
  it("clearFilters restores all defaults (empty = show all)", () => {
    useUIStore.getState().toggleType("plan");
    useUIStore.getState().toggleType("feature");
    useUIStore.getState().toggleStatusColour("red");
    useUIStore.getState().clearFilters();

    const state = useUIStore.getState();
    expect(state.activeTypes.size).toBe(0);
    expect(state.activeStatusColours.size).toBe(0);
  });
